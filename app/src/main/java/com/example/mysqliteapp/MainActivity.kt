...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        edtEmail = findViewById(R.id.mEdtEmal)
        edtNumber = findViewById(R.id.mEdtNumber)
        btnSave = findViewById(R.id.mEdtSave)
        btnView = findViewById(R.id.mEdtView)
        btnDelete = findViewById(R.id.mEdtDelete)
        ...
        btnSave.setOnClickListener {
            ...
            // check if the user is submititing empty fields
        ...
        btnDelete.setOnClickListener {
          var idNumber = edtNumber.text.toString().trim()
            if (idNumber.isEmpty()){
                ...
                var cursor = db.rawQuery("SELECT * FROM users WHERE kitambuisho=''",null)
                if (cursor.count == 0){
                    message("NO RECORD FOUND!!!", "Sorry, there's no user with provided credentials")
                }else{
